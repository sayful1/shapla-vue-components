    ...
    <div class="shapla-data-table-container">
        <table :class="tableClasses">
            ...
            <template v-if="items.length">
                <tr v-for="row in items" :key="row[index]" :class="{'is-selected':checkedItems.includes(row[index])}">
                        ...
                        <span @click="handleSelectItem(row)">
                            <slot name="check-box" :row="row">
                                ...
                                <input type="checkbox" :id="`cb-select-${row[index]}`" :value="row[index]"
                                       :checked="checkedItems.includes(row[index])">
                            </slot>
                        </span>
                    </td>
        ...
        methods: {
            ...
            handleSelectItem(item) {
                let value = item[this.index] !== undefined ? item[this.index] : item.id,
                    index = this.checkedItems.indexOf(value);
                if (-1 === index) {
            ...
            handleSelectAll() {
                ...
                if (this.items.length && (this.checkedItems.length !== this.items.length)) {
